...
def test_load_dump_properties_from_dict():
    ...
    materials, constructions, construction_sets, schedule_type_limits, \
        ...
        model.properties.energy.load_properties_from_dict(model_dict)

    new_prop_dict = model.properties.energy.dump_properties_to_dict(
        materials.values(), constructions.values(), construction_sets.values(),
        schedule_type_limits.values(), schedules.values(), program_types.values(),
        hvacs.values(), shws.values())

    for key in new_prop_dict:
        if isinstance(new_prop_dict[key], list):
            for item in model_dict['properties']['energy'][key]:
